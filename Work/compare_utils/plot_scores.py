...
from consts import CSV_VALUES_LABELS, T_Z, T_Y, T_X, R_Z, R_Y, R_X, THETA, VALIDATION_CSV_VALUES_LABELS
from my_utils.csv_files_tools import read_csv


def plot_diff_each_param_no_translation(folder, file_list):
    """
    Plot csv scores, each param of 3DoF in a different axes
    :param folder: the directory of the files
    :param file_list: file names list to compare
    :return: Nan - show data
    """
    fields = VALIDATION_CSV_VALUES_LABELS
    csv_values = []
    csv_lengths = []
    ...
    for i in range(len(fields)):
        for csv_index in range(len(csv_values)):
            ys = csv_values[csv_index][i]
            for j, y in enumerate(ys):
                if y < 0 and y < -np.pi + 0.5:
                    ys[j] = y + np.pi
                if y > 0 and y > np.pi - 0.5:
                    ys[j] = y - np.pi
...
def plot_diff(folder, filename, title=''):
    ...
    print('Avg theta diff: ' + str(np.mean(theta)))


def plot_diff_no_translation(folder, filename, title=''):
    """
    Plot the difference file scores
    :param folder: the directory of the files
    :param filename: the difference file name to compare
    ...
    # show theta result
    ax2.plot(x_s, theta, label=THETA, alpha=0.5)
    ax2.grid(True)
    ax2.legend()
