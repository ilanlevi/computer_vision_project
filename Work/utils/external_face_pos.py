...
import numpy as np
import cv2
from ..consts.facial_landmarks import FacialLandmarksConsts as flc
...
class PreProcessDataExternal:
    ...
    def align_get_matrix(self, image, gray, rect):
        # convert the landmark (x, y)-coordinates to a NumPy array
        shape = self.predictor(gray, rect)
        shape = PreProcessDataExternal.shape_to_np(shape)
        pts = np.asmatrix(shape)
        ...
        pts = np.reshape(pts * m, -1)
        # return the aligned face
        return output, pts

    def align(self, image, gray, rect):
        return self.align_get_matrix(image, gray, rect)[0]
